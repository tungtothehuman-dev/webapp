                                        ...
                                        <td className="px-5 py-3 whitespace-nowrap" onClick={e => e.stopPropagation()}>
                                            {editingTrackId === pkg.id ? (
                                                ...
                                                <div className="flex items-center gap-2 max-w-[240px] group/track">
                                                    ...
                                                    {pkg.masterTracking && pkg.masterTracking.length > 5 && (
                                                        <button 
                                                            onClick={async (e) => {
                                                                e.stopPropagation();
                                                                const btn = e.currentTarget;
                                                                btn.classList.add('animate-spin', 'text-teal-600');
                                                                try {
                                                                    const res = await fetch('/api/17track/sync', {
                                                                        method: 'POST',
                                                                        headers: {'Content-Type': 'application/json'},
                                                                        body: JSON.stringify({ trackingNumber: pkg.masterTracking, packageId: pkg.id })
                                                                    });
                                                                    const data = await res.json();
                                                                    if (data.success && data.updates) {
                                                                        updatePackage(pkg.id, data.updates);
                                                                    } else if (data.error && !data.error.includes('No status changes')) {
                                                                        // ignored silently if error is 'No status changes'
                                                                    }
                                                                } finally {
                                                                    setTimeout(() => btn.classList.remove('animate-spin', 'text-teal-600'), 500);
                                                                }
                                                            }} 
                                                            className="p-1.5 text-slate-400 hover:text-teal-600 opacity-0 group-hover/track:opacity-100 transition-all rounded-md hover:bg-teal-50 shrink-0"
                                                            title="Đồng bộ tức thì trạng thái 17Track (Bỏ qua thời gian đợi webhook tự động)"
                                                        >
