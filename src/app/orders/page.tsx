                            ...
                            <button
                              onClick={() => {
                                  if (order.Status !== 'Đã Hủy') {
                                      downloadBarcodeAsPdf(order.Description!, order["Receiver Name"] || "", order.HUB || order.Hub || "-")
                                  }
                              }}
                              disabled={order.Status === 'Đã Hủy'}
                              className={`shrink-0 p-1.5 rounded-md border transition-colors ${order.Status === 'Đã Hủy' ? 'opacity-50 cursor-not-allowed bg-slate-100 text-slate-400 border-slate-200' : 'bg-white hover:bg-indigo-600 text-slate-400 hover:text-white border-slate-200'}`}
                              title={order.Status === 'Đã Hủy' ? "Đơn đã hủy, không thể in" : "In Barcode Khổ A7"}
                            >
            ...
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <div className="flex items-center gap-4 text-slate-800">
                ...
                <button 
                  ...
                  className="px-3 py-1 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200 hover:border-indigo-500 rounded-lg text-xs font-bold uppercase transition flex items-center gap-1 shadow-sm cursor-pointer"
                  title="Tải lên PDF Label mới để ghi đè"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                  Thay Label
                </button>
                {(selectedDetail.pdfUrl || selectedDetail.pdfBase64) && selectedDetail.Status !== 'Đã Hủy' && (
                  <button 
                    onClick={() => {
                        handleClearManualLabel(selectedDetail.id, selectedDetail.Description || 'KhongMa');
                        setSelectedDetail({ ...selectedDetail, pdfUrl: '', pdfBase64: '', TrackingNumber: '' });
                    }} 
                    className="px-3 py-1 bg-amber-50 text-amber-600 hover:bg-amber-100 border border-amber-200 hover:border-amber-500 rounded-lg text-xs font-bold uppercase transition flex items-center gap-1 shadow-sm cursor-pointer"
                    title="Xóa nhãn PDF hiện tại ra khỏi hồ sơ đơn"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    Xóa Label
                  </button>
